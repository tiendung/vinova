...
var $count = 0;
vinova.log = function (a) {
  if (a === "turn off") { $logging = false; return; }
  if (a === "turn on" ) { $logging = true;  return; }
  ...
  if (charCode === SWITCH || charCode === BACKSPACE || this.raw.length > 1) {
    if ( $viMode !== VNI ) {
      ...
      if ( this.enFilterIsNotUsed )  
      {
        ...
        else if ( typeof( $evConfuse ) != "undefined" && $evConfuse[ raw ] ) 
        { 
          $viMode = $preViMode; 
          this.enFilterIsNotUsed = false;
        }
...
/*qvfxj*/ var $enFilter = {'of':1,'if':1,'ex':1,'move':1,'love':1,'navy':1,'nix':1,'nova':1,'now':1,'ova':1,'oven':1,'room':1,'rove':1,'see':1,'sofa':1,'soon':1,'taxi':1,'viva':1,'how':1,'':1,'':1,'':1,'':1 };
//*telex*/ 
